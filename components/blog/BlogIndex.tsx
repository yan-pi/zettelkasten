...
export const BlogIndex = ({ maxItems }: { maxItems?: number }) => (
  <div>
    {(getPagesUnderRoute('/blog') as Array<Page & { frontMatter: any }>).slice(0, maxItems).map((page) => (
      <Link key={page.route} href={page.route} className="flex justify-between items-center mb-8 group">
        <div className="flex items-center justify-between">
          <h2 className="block font-bold text-2xl opacity-90 group-hover:opacity-100">
